...
def on_quit_clicked(icon):
    icon.stop()
    try:
        subprocess.run(["taskkill", "/F", "/IM", "amazon_scrapy.exe"])
    except subprocess.CalledProcessError as e:
        print(f"Failed to kill process. Error: {e}")

def new():
    img_path = r'icon/amazon.ico'
    ...
    icon = pystray.Icon("amazon.ico", image, "メルカリツール", menu)
